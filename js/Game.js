class Game {
  ...
  start() {
   ...
   // Creating groups 
    coinGrp = new Group();
    oilGrp = new Group();
    this.addSprites(coinGrp, 12, coinImg, 0.6);
    this.addSprites(oilGrp, 11, oilCanImg, 0.7);
  }
  ...
  play() {
    ...
    if (keyIsDown(LEFT_ARROW)&& car1.x> width/3 - 50) {
      car1.x -= 5;
    }

    //For moving the car in Right direction
    if (keyIsDown(RIGHT_ARROW)&& car1.x <width/2 +300) {
      car1.x += 5;
    }}
    
    // Funtion used to see the position of our car
    camera.position.y = car1.position.y;
    fill("green");
    ellipse(car1.x,car1.y,50,50);
   
    if(car1.y <-2900|| car2.y <- 2900){
     gameState = 2; 
      if(car1.y <- 2900){
        // Adding some Sweet Alerts
       swal(
          {
            title: `Yayy!! You Win`,
            text: "Wanna Try again? If Yes the click down ",
            imageUrl:
              "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR9H30hgL-TFaut6-QrqGfqXl0Maa_pNgWLsg&usqp=CAU",
            imageSize: "150x150",
        ...
        swal(
          {
            title: `Sorry! You Lose the game!`,
            text: "Wanna Try again? If Yes the click down ",
            imageUrl:
              "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTRqT5N7LBZmBR7-pXioszVY_Rpti32dI5VXw&usqp=CAU",
            imageSize: "150x150",
            confirmButtonText: "YES",
          },
          function (isConfirm) {
  ...
  addSprites(spritegroup, numberOfSprites, spriteImage, spriteScale) {
    for (var i = 0; i < numberOfSprites; i++) {
      ...
      Sprite.setCollider("rectangle",0,0,80,70);
